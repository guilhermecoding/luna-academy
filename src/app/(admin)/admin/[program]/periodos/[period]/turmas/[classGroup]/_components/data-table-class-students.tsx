...
export function DataTableClassStudents<TData, TValue>({
    ...
    const handleUnlink = () => {
        ...
        startTransition(async () => {
            ...
            const res = await unlinkStudentsFromClassGroupAction(studentIds, classGroupId, periodId, adminPassword);

            if (res.success) {
                toast.success("Alunos desvinculados da turma com sucesso!");
                setRowSelection({});
                    ...
                    {hasSelection && (
                        <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto animate-in fade-in zoom-in duration-200">
                            ...
                            >
                                ...
                                Desvincular da Turma ({selectedRows.length})
                            </Button>
                        </div>
                    )}
                </div>
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Você está prestes a desvincular <b>{selectedRows.length} aluno{selectedRows.length > 1 ? "s" : ""}</b> desta turma.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="bg-destructive/10 p-4 rounded-xl space-y-2">
                        ...
                        <p className="text-sm text-destructive/80 leading-relaxed">
                            Esta ação removerá as matrículas dos alunos em <b>todas as disciplinas</b> vinculadas a esta turma. Notas e presenças também poderão ser apagadas se houverem. Se o aluno não estiver matriculado em nenhuma outra turma neste período, seu status mudará para &quot;Em Espera&quot;.
                        </p>
                    </div>
