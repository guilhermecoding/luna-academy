...
import { COURSE_TEACHERS_EXPORT_COLUMNS } from "@/services/export/teachers-export.config";
import { getTeachersByCourseForExport } from "@/services/export/teachers-csv.export";
...
export const metadata: Metadata = {
    title: "Pré-visualização PDF - Professores da Disciplina",
};

async function CourseTeachersExportPreviewPageContent({
    params,
}: PageProps<"/admin/[program]/periodos/[period]/turmas/[classGroup]/disciplinas/[course]/professores/export-preview">) {
    ...
    const rows = await getTeachersByCourseForExport(courseData.id);

    const generatedAt = formatExportGeneratedAt();
    const exportPdfUrl = `/api/admin/${program}/periodos/${period}/turmas/${classGroupSlug}/disciplinas/${courseCode}/professores/export?format=pdf`;
        ...
        <Page className="flex flex-col min-h-screen">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <TitlePage
                            ...
                            description={`Professores da disciplina ${courseData.name}`}
                        />
                    </div>
            ...
            <Section className="flex-1 mt-4 min-h-0">
                ...
                <div className="rounded-2xl border border-surface-border overflow-hidden bg-muted/30" style={{ height: "calc(100vh - 220px)" }}>
                    <TeachersPdfPreview
                        title={`Professores de ${courseData.name}`}
...
export default function CourseTeachersExportPreviewPage({
    params,
    searchParams,
}: PageProps<"/admin/[program]/periodos/[period]/turmas/[classGroup]/disciplinas/[course]/professores/export-preview">) {
    return (
        <Suspense fallback={<PageSkeleton />}>
            <CourseTeachersExportPreviewPageContent params={params} searchParams={searchParams} />
        </Suspense>
    );
}
