...
const selectColumn: ColumnDef<StudentPeriodListItem> = {
    id: "select",
    header: ({ table }) => (
        <Checkbox
...
function buildPeriodStudentBaseColumns(): ColumnDef<StudentPeriodListItem>[] {
    return [
        {
            accessorKey: "avatar",
            header: "",
...
export function createPeriodStudentColumns(opts: {
    ...
    const { canWrite, onTurmasClick } = opts;
    const baseColumns = buildPeriodStudentBaseColumns();
                    ...
                    return (
                        <div className="flex justify-end">
                            <Button
                                ...
                                variant="ghost"
                                size="icon-sm"
                                aria-label="Turmas matriculadas"
                                onClick={() => onTurmasClick(row.original)}
                            >
                                <IconUsers className="size-5 text-muted-foreground shrink-0" />
                            </Button>
                        </div>
                    );
                },
...
export function usePeriodStudentColumns(opts: {
    onTurmasClick: (student: StudentPeriodListItem) => void;
}): ColumnDef<StudentPeriodListItem>[] {
    const canWrite = useCanWrite();
    return useMemo(
        () => createPeriodStudentColumns({ canWrite, onTurmasClick: opts.onTurmasClick }),
        [canWrite, opts.onTurmasClick],
    );
}

function buildClassGroupStudentBaseColumns(): ColumnDef<StudentListItem>[] {
    return [
        {
            accessorKey: "avatar",
            header: "",
...
const classGroupSelectColumn: ColumnDef<StudentListItem> = {
    id: "select",
    header: ({ table }) => (
        <Checkbox
            checked={table.getIsAllPageRowsSelected()}
...
export function createClassGroupStudentColumns(canWrite: boolean): ColumnDef<StudentListItem>[] {
    const baseColumns = buildClassGroupStudentBaseColumns();
...
export function useClassGroupStudentColumns(): ColumnDef<StudentListItem>[] {
    const canWrite = useCanWrite();
    return useMemo(() => createClassGroupStudentColumns(canWrite), [canWrite]);
}

/** @deprecated Use createClassGroupStudentColumns or useClassGroupStudentColumns */
export const classGroupStudentsColumns = createClassGroupStudentColumns(true);
